...
import os
from Asset_Stress_Thaniie import run_Asset_Stress_Test


#%% 01_variable_initialization
...
#%% 03_selectors_default

col1, col2 = st.columns([2,2], gap="small")

with col1:
    st.subheader('Input parameters', divider='green')
...
with col2:
    st.subheader('PALS & schedules folders', divider='green') 
...
if st.button('Run Asset Stress Test'):
    if file_path and output_path_user:
        try:
            message = run_Asset_Stress_Test(file_path, output_path_user, val_date )
            st.success(f"✅ Asset Stress Test ran successfully.")
            ...
            if os.path.exists(message):
                ...
                st.error('Asset Stress Test output file not found', icon="🚨")
        except Exception as e:
            st.error(f"🚨 An error occured during Asset Stress Test: {e}")
    else:
